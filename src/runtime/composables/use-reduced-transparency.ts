...
export const useReducedTransparency = () => {
  const selectedReducedTransparency = useStorage('material-yu:reduced-transparency', () => 'system')
  ...
  const currentReducedTransparency = computed(() => {
    if (selectedReducedTransparency.value === 'system') return isSystemReducedTransparency.value ? 'reduced' : 'unreduced'
